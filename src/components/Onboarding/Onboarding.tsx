...
const imageList = [
  "/onboarding/custom-brand-assets.png",
  "/onboarding/redemption-analytics.png",
  "/onboarding/krisflyer.svg",
  "/onboarding/velocity.svg",
];

function preloadImages(imageUrls: string[], callback: () => void) {
  let loaded = 0;
...
function trackAnimations(
  ...
  document.querySelectorAll(selector).forEach((el) => {
    el.addEventListener("animationend", () => {
      completedAnimations++;
      if (completedAnimations === total) callback();
    });
  });
}

export default function AscendaLogoReveal() {
    ...
    requestAnimationFrame(() => {
      ...
      if (line instanceof SVGPathElement) {
        document.documentElement.style.setProperty(
          "--total-length",
          line.getTotalLength().toString()
        );
      }

      preloadImages(imageList, () => {
        trackAnimations("#ascenda-logo", 9, () => {
          setShowLanding(true);
        });
      });
    });
  ...
  function enableParallax() {
    ...
    function handleMouseMove(e: MouseEvent) {
      ...
      tiles.forEach((tile) => {
        const speed = Number(tile.getAttribute("data-speed")) * 2 || 10;
  ...
  return (
    <div ref={containerRef} id="app">
      {showLanding ? <Landing onRestart={logoReveal} /> : <Logo />}
    </div>
  );
}

function Landing({ onRestart }: { onRestart: () => void }) {
  return (
    <>
      <div className=" relative w-screen h-screen overflow-hidden grid place-content-center text-cloudburst-500">
        {/* Main welcome tile */}
        <div
          id="welcome"
          className={`max-w-[540px] m-4 flex flex-col items-center gap-6.25 ${styles.welcome}`}
        >
          <h1 className="text-2xl md:text-4xl font-bold">Welcome [f_name]</h1>
          <p className="text-center">
            Lorem ipsum dolor sit amet consectetur. Facilisis nisl cursus vitae
            ridiculus quis facilisi mauris. Tristique scelerisque arcu facilisi
            at augue facilisis amet mattis donec. Sapien semper eu tristique
            tellus facilisis. Sit ut fermentum faucibus ac facilisi id egestas
            ultrices.
          </p>
          <button
            onClick={onRestart}
            className="py-3 px-6 bg-cloudburst-500 text-sm md:text-base text-white font-bold rounded flex justify-between w-[200px] cursor-pointer hover:bg-cloudburst-400 active:bg-cloudburst-700 focus-visible:outline-shakesphere-300 focus-within:outline-2"
          >
            Get started
            <svg
            ...
            >
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M4.98145 13.1763H19.4814V11.1763H4.98145V13.1763Z"
              />
              <path
                fillRule="evenodd"
                clipRule="evenodd"
        ...
        {/* You can break these out into their own components if needed */}

        {/*Custom brand tile*/}
        <div
          id="custom-brand"
          data-speed="36"
          className={`tile absolute pointer-events-none rounded-2xl bg-white p-4.25 flex flex-col gap-3 items-center shadow-ascenda shadow-cloudburst-500/10 max-[1024px]:hidden top-[30%] left-[2.5%] ${styles.custombrand}`}
        >
          <h5 className="font-bold">Custom brand assets</h5>
          <img width="180" src="/onboarding/custom-brand-assets.png" alt="" />
        </div>
        {/* Redemption analytics tile */}
        <div
          id="redemption-analytics"
          data-speed="24"
          className={`${styles.redemptionanalytics} tile absolute pointer-events-none rounded-2xl bg-white p-4.25 flex flex-col gap-3 items-center shadow-ascenda shadow-cloudburst-500/10 bottom-[-5%] left-[5%]`}
        >
          <h5 className="font-bold">Redemption analytics</h5>
          <img width="140" src="/onboarding/redemption-analytics.png" alt="" />
        </div>
        {/*Points to cash tile*/}
        <div
          id="points-cash"
          data-speed="20"
          className={`${styles.pointscash} tile absolute pointer-events-none max-w-[310px] rounded-2xl bg-white p-4.25 flex flex-col gap-3 items-center shadow-ascenda shadow-cloudburst-500/10 top-[-5%] left-[15%] md:top-[2.5%] md:left-[25%]`}
        >
          <h5 className="font-bold">Points to cash</h5>
          <p className="text-sm">
            Determine increment percentage and maximum cash percentage allowed.
          </p>
          <div id="slider" className="flex items-center gap-2.5 w-full ">
            <span className="text-cloudburst-400">0%</span>
            <div className="h-[4px] grow bg-cloudburst-150 rounded relative before:absolute before:content-[''] before:rounded before:left-0 before:top-0 before:w-2/5 before:h-full before:bg-shakesphere-500 after:absolute after:w-3 after:h-3 after:rounded-full after:bg-shakesphere-500 after:-top-1 after:left-[35%]"></div>
            <span className="text-cloudburst-400">100%</span>
          </div>
        </div>
        ...
        <div
          id="virgin-australia"
          data-speed="36"
          className={`${styles.virginaustralia} tile absolute pointer-events-none rounded-2xl bg-white p-4.25 flex gap-4 items-center shadow-ascenda shadow-cloudburst-500/10 min-w-[360px] max-[882px]:hidden right-[-1.5%] top-[12.5%]`}
        >
          <img src="/onboarding/velocity.svg" alt="" />
          <div className="flex flex-col">
            <h5 className="font-bold">Virgin Australia</h5>
            <span className="text-sm text-cloudburst-400 uppercase">
              Transfer partners
            </span>
          </div>
        </div>
        ...
        <div
          ...
          data-speed="24"
          className={`${styles.singaporeairlines} tile absolute pointer-events-none rounded-2xl bg-white p-4.25 flex gap-4 items-center shadow-ascenda shadow-cloudburst-500/10 min-w-[360px] right-[-4.5%] top-[15%] md:top-[25%]`}
        >
          <img src="/onboarding/krisflyer.svg" alt="" />
          <div className="flex flex-col">
            <h5 className="font-bold">Singapore Airlines</h5>
            <span className="text-sm text-cloudburst-400 uppercase">
              Transfer partners
            </span>
          </div>
        ...
        <div
          id="eligible-users"
          data-speed="36"
          className={`${styles.eligibleusers} tile absolute pointer-events-none rounded-2xl bg-white p-4.25 flex flex-col gap-4 shadow-ascenda shadow-cloudburst-500/10 min-w-[360px] max-[596px]:hidden bottom-[10%] right-[-1.5%]`}
        >
          <h5 className="font-bold">581,900 Eligible users</h5>
          <div className="grid use grid-cols-[auto_auto_auto] gap-3">
            <span className="bg-cloudburst-200 text-sm px-1.5 py-0.5 rounded-sm w-fit h-fit">
              Product type
            </span>
            <span className="text-cloudburst-400 uppercase text-sm">
              Equals
            </span>
            <span className="text-cloudburst-400 text-sm">Rewards Card</span>

            <span className="bg-cloudburst-200 text-sm px-1.5 py-0.5 rounded-sm w-fit h-fit">
              User age
            </span>
            <span className="text-cloudburst-400 text-sm uppercase">
              Is greater than
            </span>
            <span className="text-cloudburst-400 text-sm">25</span>
          </div>
        </div>
      </div>
    </>
...
function Logo() {
  return (
    <div className="w-screen h-screen grid place-content-center">
      <div
        id="ascenda-logo"
        className={`relative overflow-hidden w-[324px] h-[65px] ${styles.ascendalogo} fill-cloudburst-500`}
      >
        <svg
          className={`${styles.aa}`}
          id="aa"
        ...
        >
          <clipPath id="clip-shape">
            <path d="M26.0276 0.447898H35.2348L61.0021 59.8116H48.8342L43.1732 46.1215C35.7771 55.2482 25.0732 59.8116 11.0617 59.8116H0L26.0276 0.415405V0.447898ZM39.1498 36.1896L30.3547 12.99L13.8705 50.5404C18.9515 50.6619 23.9726 49.4241 28.4134 46.9554C32.9084 44.4155 36.6185 40.6915 39.139 36.1896H39.1498Z" />
          </clipPath>
        </svg>
        <svg
        ...
        >
          <path
            ...
            id="line"
            d="M41.23,41.6c-1.2.5-1.93,1.57-2.89,2.5-6.85,6.61-14.18,12.09-31.47,12.09C24.87,15.78,30.7,2.02,30.87,1.68c0,0,22,53,26,62.5"
            stroke="currentColor"
            stroke-width="11.5"
